...
function Header({
  ...
  return (
    <View style={s.headerBar} fixed>
      <Text style={s.headerWordmark}>NAIB</Text>
  ...
  return (
    <View style={s.footer} fixed>
      <Text style={s.footerLeft}>
        NAIB — NextGen Aptitude & Integrity Battery | Confidential
      </Text>
      <Text
        style={s.footerRight}
  ...
  return (
    <View>
      ...
      <Text style={[s.bodySmall, { textAlign: "center", marginTop: 6 }]}>
        This scorecard was generated by the NAIB platform. Assessment results
        should be used as one data point among many in the hiring decision. All
        scores are norm-referenced against the NAIB manufacturing population.
      </Text>
    </View>
  );
}
...
export function PDFScorecard({ candidate }: PDFScorecardProps) {
  ...
  return (
    <Document
      title={`NAIB Scorecard — ${candidateName}`}
      author="NAIB Platform"
      subject={`Candidate Assessment Scorecard for ${candidateName}`}
      creator="NAIB — NextGen Aptitude & Integrity Battery"
    >
      {/* Page 1: Executive Summary */}
